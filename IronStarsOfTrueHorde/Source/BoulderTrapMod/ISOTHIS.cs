...
namespace ISOTH
{
    public class ISOTHIS : Projectile
    {
        ...
        private static readonly Material ballimage1 = MaterialPool.MatFrom("Things/ISOTH_0");

        private static readonly Material ballimage2 = MaterialPool.MatFrom("Things/ISOTH_1");

        private static readonly Material ballimage3 = MaterialPool.MatFrom("Things/ISOTH_2");
        ...
        public override void Tick()
        {
            ...
            if (this.Spawned)
            {
                ...
                if (tickCounter % 5 == 0)
                {
                    MoteMaker.ThrowFireGlow(this.Position, this.Map, 1f);
                ...
                if (tickCounter % 10 == 0)
                {
                    ...
                    GenSpawn.Spawn(ThingMaker.MakeThing(ThingDef.Named("Filth_Ash"), null), this.Position, this.Map);
                    IntVec3 tempfuel;
                    CellFinder.TryFindRandomCellNear(base.Position, base.Map, 2, null, out tempfuel, -1);
                    GenSpawn.Spawn(ThingMaker.MakeThing(ThingDef.Named("Filth_Fuel"), null), tempfuel, this.Map);
                }
                ...
                if (tickCounter <= 0)
                {
                    ...
                    IntVec3 c;
                    CellFinder.TryFindRandomCellNear(this.Position, base.Map, 2, null, out c, -1);
                    FireUtility.TryStartFireIn(this.Position, base.Map, Rand.Range(0.2f, 0.925f));
        ...
        private void Stopatwall()
        {
            ...
            for (int i = 0; i < thingList.Count; i++)
            {
                ...
                if (thing.def.category == ThingCategory.Building)
                {
                    ...
                    if (flag)
                    {
                        if(wallcrashCounter > 0)
                        {
                            wallcrashCounter--;
                            //Verse.Log.Message("c "+ wallcrashCounter.ToString(), true);
                        }
                        else
                        {
        ...
        {
            int num = GenRadial.NumCellsInRadius(2.5f);
            for (int i = 0; i < num; i++)
            {
                ...
                if (this.Map != null)
                {
                    if (intVec.InBounds(this.Map))
                    {
                        ...
                        if (firstPawn == null || !firstPawn.Downed || !Rand.Bool)
                        {
                            ...
                            this.DoDamage(intVec, damageFactor, CorC);
                        }
                    }
                }
            }
