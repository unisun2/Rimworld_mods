...
namespace FP_RSLUM
{
    class PawnColumnWorker_AGL : PawnColumnWorker
    {
        ...
        {
            return "PawnColumnWorker_AGL_Tip_Desc".Translate();
        }
            ...
            if (Widgets.ButtonImage(rect, harmony_patches.DistributeIMG, Color.white, GenUI.SubtleMouseoverColor))
            {
                ...
                if (pawnlvcomp != null)
                {
                    ...
                    else if (Input.GetKey(KeyCode.LeftAlt))
                    {
                        if (Input.GetKey(KeyCode.LeftShift))
                        {
                            int tempv = Math.Min(pawnlvcomp.AGL - FP_RSLUM_setting.Startingstat_min, 100);
                            if (pawnlvcomp.AGL > FP_RSLUM_setting.Startingstat_min)
                            {
                                pawnlvcomp.StatPoint += tempv;
                                pawnlvcomp.AGL -= tempv;
                            }

                        }
                        else if (Input.GetKey(KeyCode.LeftShift))
                        {
                            int tempv = Math.Min(pawnlvcomp.AGL - FP_RSLUM_setting.Startingstat_min, 10);
                            if (pawnlvcomp.AGL > FP_RSLUM_setting.Startingstat_min)
                            {
                                pawnlvcomp.StatPoint += tempv;
                                pawnlvcomp.AGL -= tempv;
