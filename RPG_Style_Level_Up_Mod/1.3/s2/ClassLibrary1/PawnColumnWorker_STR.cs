        ...
        public override void DoCell(Rect rect, Pawn pawn, PawnTable table)
        {
            ...
            if (pawnlvcomp != null)
            {
                ...
                else
                {
                    ...
                    statCheckbox(topLeft, ref pawnlvcomp.STRauto, 30f, def.paintable, pawnlvcomp);
                    Rect rect2 = new Rect(rect.x + 35, rect.y, rect.width - 35, Mathf.Min(rect.height, 30f));
                    String textFor = this.GetTextFor(pawn);
                    if (textFor != null)
                    {
            ...
            if (Widgets.ButtonImage(rect, harmony_patches.DistributeIMG, Color.white, GenUI.SubtleMouseoverColor))
            {
                ...
                if (pawnlvcomp != null)
                {
                    ...
                    else if (Input.GetKey(KeyCode.LeftAlt))
                    {
                        if (Input.GetKey(KeyCode.LeftShift))
                        {
                            int tempv = Math.Min(pawnlvcomp.STR - FP_RSLUM_setting.Startingstat_min, 10);
                            if (pawnlvcomp.STR > FP_RSLUM_setting.Startingstat_min)
                            {
                                pawnlvcomp.StatPoint += tempv;
                                pawnlvcomp.STR -= tempv;
                            }
                        }
                        else if (Input.GetKey(KeyCode.LeftControl))
                        {
                            int tempv = Math.Min(pawnlvcomp.STR - FP_RSLUM_setting.Startingstat_min, 100);
                            if (pawnlvcomp.STR > FP_RSLUM_setting.Startingstat_min)
                            {
                                pawnlvcomp.StatPoint += tempv;
                                pawnlvcomp.STR -= tempv;
